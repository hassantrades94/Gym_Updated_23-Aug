...
-- FIXED VERSION: Handles missing reference_id column gracefully
...
CREATE INDEX IF NOT EXISTS idx_gym_wallets_gym_id ON gym_wallets(gym_id);
...
CREATE INDEX IF NOT EXISTS idx_wallet_transactions_reference ON wallet_transactions(reference_id) WHERE reference_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_wallet_transactions_razorpay ON wallet_transactions(razorpay_payment_id) WHERE razorpay_payment_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_wallet_transaction_history_gym_date ON wallet_transaction_history(gym_id, transaction_date);
CREATE INDEX IF NOT EXISTS idx_wallet_transaction_history_original_id ON wallet_transaction_history(original_transaction_id);

-- Unique constraints for preventing duplicate transactions
CREATE UNIQUE INDEX IF NOT EXISTS uq_wallet_transactions_razorpay_id 
...
-- Create triggers
DROP TRIGGER IF EXISTS validate_wallet_transaction_trigger ON wallet_transactions;
...
DROP TRIGGER IF EXISTS update_gym_wallets_updated_at ON gym_wallets;
...
SELECT id, 0.00
FROM gyms
WHERE id NOT IN (SELECT gym_id FROM gym_wallets WHERE gym_id IS NOT NULL);

-- Update existing wallet_transactions records to have reference_id if missing
...
SELECT COUNT(*) as gym_wallets_count FROM gym_wallets;
